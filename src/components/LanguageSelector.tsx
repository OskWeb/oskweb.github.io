...
export const LanguageSelector = ({ languages, currentUrl, initialLang }) => {
    ...
    const changeLanguage = (value) => {
        ...
        window.location.href = `/${value}` + currentUrl;
    };

    const getLanguageNameByCode = () => {
        const language = Object.entries(languages).find(([code, name]) => code === lang);
        return language?.[1] as string;
    }
    ...
    return (
        <div className="languageSelectorBox">
            <div className="selected" onClick={() => setOpen(!open)}>
                ...
                <span className='languageSelected'>
            ...
            <ul className={`dropdown-options + ${open ? 'dropdown-open' : ''}`} >
                {
                    Object.entries(languages).map(([code, name]) => (
                        <li key={code} value={code} onClick={() => changeLanguage(code)}>
                            {name as string}
                        </li>
                    ))
                }
